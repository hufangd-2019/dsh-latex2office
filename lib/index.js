/**
 * dsh-latex2office — LaTeX → Office formula tools (v0.1.0)
 *
 ...
 *   3. known local paths table (this machine's install locations)
...
const FORMULAS_DESC =
  'Array of formula specs. Each item: {latex: string (required; LaTeX code WITHOUT $$/$ delimiters), ' +
...
function normalizeFormulas(raw) {
  ...
  for (let i = 0; i < raw.length; i++) {
    const f = raw[i] || {}
    const item = {
      latex: String(f.latex == null ? '' : f.latex).trim(),
  ...
  apply(ctx, config) {
    ...
    ctx.effect(() =>
      ctx.tools.register({
        ...
        parameters: {
          file: { type: 'string', required: true, description: 'Absolute path of the target .docx or .pptx file (must exist)' },
          formulas: { type: 'array', required: true, description: FORMULAS_DESC },
    ...
    ctx.effect(() =>
      ctx.tools.register({
        ...
        parameters: {
          output: { type: 'string', required: true, description: 'Absolute path of the .docx file to create (parent directory must exist; existing file is overwritten only if overwrite:true)' },
          title: { type: 'string', description: 'Optional document title (Heading 1)' },
          formulas: {
            type: 'array',
            required: true,
            description:
              'Array of {latex: string (required, WITHOUT $$ delimiters), number?: string (e.g. "3.1"), display?: boolean (default true)}',
          },
    ...
    ctx.effect(() =>
      ctx.tools.register({
        ...
        parameters: {
          latex: { type: 'string', required: true, description: 'LaTeX code WITHOUT $$/$ delimiters' },
        ...
        async execute(args, exec) {
          try {
            const latex = String(args.latex == null ? '' : args.latex).trim()
            ...
            const request = {
              ...
              preview_dir:
                cwdRef && cwdRef !== '.'
                  ? String(cwdRef).replace(/[\\/]+$/, '') + '/latex2office-previews'
                  : undefined,
              renderTimeoutSec: Number(cfg.renderTimeoutSec) > 0 ? Number(cfg.renderTimeoutSec) : 90,
            }
